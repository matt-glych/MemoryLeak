...
public class AgentBehaviours
{
    ...
    public AgentBehaviours(EnemyAgent agent)
    {
        owner = agent;
    ...
    public void AnimateFromVelocity()
    {
       /* float velocity = nav.velocity.magnitude;
        //float velocity = nav.speed;
        if (velocity >= 1.8f)
        {
            owner.ChangeAnimationState("run");
        }
        else if (velocity >= 0.5f && velocity < 1.8f)
        {
        ...
        //Debug.Log("Velocity magnitude: " + nav.velocity.magnitude);
    }

    public void SetWalking() { nav.speed = owner.walkSpeed; }
    public void SetRunning() { nav.speed = owner.runSpeed; }

    // true if within a stopping distance of a target position
        ...
        if (anomalySpotted)
        {
            ...
            if (seconds >= owner.spotTargetDelay)
            {
                ...
                else
                {
                    ...
                    owner.currentState = EnemyState.Patrol;
                }
            }
        }
    }
